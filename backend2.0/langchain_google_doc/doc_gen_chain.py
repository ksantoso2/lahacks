...
from .tools import CreateGoogleDocTool # Assuming the tool is defined
...
async def run_generation_with_hitl(topic: str, access_token: str) -> str:
    ...
    print(generated_content[:500] + "..." if len(generated_content) > 500 else generated_content) # Preview content
    print("=" * 30)
    ...
    # TODO: Modify the tool/service to actually use the 'generated_content'
    doc_tool = CreateGoogleDocTool()
    try:
        creation_result = await doc_tool.arun(
            title=generated_title,
            content=generated_content, # Tool needs to be updated to use this
